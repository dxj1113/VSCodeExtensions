...
function activate(context) {
    ...
    if (process.platform == "darwin") {
        prevCrashFile = "";
        let crashFolder = path.resolve(context.storagePath, "../../../../../../Logs/DiagnosticReports");
        fs.stat(crashFolder, (err, stats) => {
            let crashObject = {};
            if (err) {
                crashObject["fs.stat: err.name"] = err.name;
            ...
            fs.watch(crashFolder, (event, filename) => {
                ...
                if (!filename.startsWith("Microsoft.VSCode.CPP."))
                    return;
                fs.readFile(path.resolve(crashFolder, filename), (err, data) => {
                    if (err) {
                        crashObject["readFile: err.name"] = err.name;
                        Telemetry.logLanguageServerEvent("MacCrash", crashObject, null);
                        return;
                    }
                    ...
                    let crashString = data.toString("utf-8", startCrash, endCrash);
                    if (crashString.length > 8192)
                        crashString = crashString.substr(0, 8189) + "...";
                    crashObject["CrashingThreadCallStack"] = crashString;
                    Telemetry.logLanguageServerEvent("MacCrash", crashObject, null);
                });
            });
        });
