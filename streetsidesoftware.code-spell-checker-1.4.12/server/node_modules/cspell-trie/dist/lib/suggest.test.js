...
describe('Validate Suggest', () => {
    it('Tests suggestions', () => {
        ...
        chai_1.expect(suggestions).to.contain('talks');
        chai_1.expect(suggestions).to.contain('talk');
        chai_1.expect(suggestions[0]).to.be.equal('talks');
        chai_1.expect(suggestions[1]).to.be.equal('talk');
        chai_1.expect(suggestions).to.deep.equal(['talks', 'talk', 'walks', 'talked', 'talker', 'walk']);
    });
    it('Tests suggestions', () => {
        const trie = trie_1.Trie.create(sampleWords);
        // cspell:ignore tallk
        ...
        chai_1.expect(suggestions).to.contain('talks');
        chai_1.expect(suggestions).to.contain('talk');
        chai_1.expect(suggestions[1]).to.be.equal('talks');
        chai_1.expect(suggestions[0]).to.be.equal('talk');
        chai_1.expect(suggestions).to.deep.equal(['talk', 'talks', 'walk']);
    });
    it('Tests suggestions', () => {
        const trie = trie_1.Trie.create(sampleWords);
        ...
        // console.log(JSON.stringify(results));
        const suggestions = results.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal(['journals', 'journal']);
    });
    it('Tests suggestions for `juornals` (reduced cost for swap)', () => {
        ...
        // console.log(JSON.stringify(results));
        const suggestions = results.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal([
            'journals',
            'journal',
            'journalism',
            'journalist',
    ...
    it('Tests suggestions', () => {
        ...
        // console.log(JSON.stringify(results));
        const suggestions = results.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal(['joyfully', 'joyful', 'joyfuller', 'joyfullest', 'joyous']);
    });
    it('Tests compound suggestions', () => {
        ...
        // console.log(JSON.stringify(results));
        const suggestions = results.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal(['walking talking joy',]);
    });
    it('Tests suggestions', () => {
        const trie = trie_1.Trie.create(sampleWords);
    ...
    it('Tests suggestions with low max num', () => {
        ...
        // console.log(JSON.stringify(results));
        const suggestions = results.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal(['joyfully', 'joyful', 'joyfuller']);
    });
    it('Tests genSuggestions', () => {
        ...
        chai_1.expect(suggestions).to.not.contain('joyfully');
        chai_1.expect(suggestions).to.deep.equal(['joyful', 'joyfuller', 'joyfullest']);
        chai_1.expect(collector.maxCost).to.be.lessThan(300);
    });
    it('Tests genSuggestions wanting 0', () => {
        ...
        collector.collect(Sug.genSuggestions(trie.root, collector.word));
        const suggestions = collector.suggestions.map(s => s.word);
        chai_1.expect(suggestions).to.be.length(0);
    });
    ...
    it('Tests genSuggestions with compounds', () => {
        ...
        collector.collect(Sug.genCompoundableSuggestions(trie.root, collector.word, Sug.CompoundWordsMethod.SEPARATE_WORDS));
        const suggestions = collector.suggestions.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal(['joyful walk', 'joyful walks', 'joyfully walk']);
        chai_1.expect(collector.maxCost).to.be.lessThan(300);
    });
    it('Tests genSuggestions with compounds', () => {
        const trie = trie_1.Trie.create(sampleWords);
        ...
        const collector = Sug.suggestionCollector('joyfullwalk', 3);
        collector.collect(Sug.genCompoundableSuggestions(trie.root, collector.word, Sug.CompoundWordsMethod.JOIN_WORDS));
        const suggestions = collector.suggestions.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal(['joyful+walk', 'joyful+walks', 'joyfully+walk',]);
        chai_1.expect(collector.maxCost).to.be.lessThan(300);
    });
    ...
    it('Test that accents are closer', () => {
        ...
        collector.collect(Sug.genCompoundableSuggestions(trie.root, collector.word, Sug.CompoundWordsMethod.JOIN_WORDS));
        const suggestions = collector.suggestions.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal(['walk', 'walks', 'talk']);
    });
    it('Test that accents are closer', () => {
        const trie = trie_1.Trie.create(sampleWords);
        ...
        collector.collect(Sug.genCompoundableSuggestions(trie.root, collector.word, Sug.CompoundWordsMethod.JOIN_WORDS));
        const suggestions = collector.suggestions.map(s => s.word);
        chai_1.expect(suggestions).to.deep.equal(['walked', 'walker', 'talked']);
    });
});
const sampleWords = [
    'walk',
